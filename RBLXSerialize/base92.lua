...
for i = 1, 91 do
	local char = string.sub(CHAR_SET, i, i)
	encode_CharSet[i - 1] = char
	decode_CharSet[char] = i - 1
end
...
local function encodeBase91(input)
	...
	for i = 1, #input do
		...
		if numBits > 13 then
			...
			output[c] = encode_CharSet[entry % 91] .. encode_CharSet[entry // 91]
			c += 1
		end
	end
	...
	if numBits > 0 then
		...
		if numBits > 7 or counter > 90 then
			output[c + 1] = encode_CharSet[counter // 91]
		end
	end
...
local function decodeBase91(input)
	...
	for i = 1, #input do
		local decodeChar = decode_CharSet[string.sub(input, i, i)]
		if decodeChar then
			if entry == -1 then
				entry = decodeChar
			else
